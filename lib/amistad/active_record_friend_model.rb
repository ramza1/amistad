...
module Amistad
  ...
  has_many  :pending_invited, -> {where('friendships.pending' => true, 'friendships.blocker_id' => nil )},
            :through => :friendships,
            :source => :friend

  has_many  :invited, -> {where('friendships.pending' => false, 'friendships.blocker_id' => nil )},
            :through => :friendships,
            :source => :friend
  ...
  has_many  :pending_invited_by, -> {where('friendships.pending' => true, 'friendships.blocker_id' => nil )},
            :through => :inverse_friendships,
            :source => :friendable

  has_many  :invited_by, -> {where('friendships.pending' => false, 'friendships.blocker_id' => nil )},
            :through => :inverse_friendships,
            :source => :friendable
  ...
  has_many  :blockades, -> {where('friend_id <> blocker_id')},
            :through => :blocked_friendships, 
            :source => :friend

  has_many  :blockades_by, -> {where('friendable_id <> blocker_id')},
            :through => :blocked_friendships,
            :source => :friendable 
    end
